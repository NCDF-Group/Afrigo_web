...
export function SectionHeading({ overline, title, text, invert = false, center = false }: { overline: string; title: string; text?: string; invert?: boolean; center?: boolean }) {
  return (
    <div data-reveal className={`max-w-2xl ${center ? 'mx-auto text-center' : ''}`}>
      <p className={`text-xs font-bold uppercase tracking-[.08em] ${invert ? 'text-gold-300' : 'text-gold-700'}`}>{overline}</p>
...
export function HeroBackdrop({ image, position = 'center', priority = true }: { image?: string; position?: string; priority?: boolean }) {
  return (
    <div aria-hidden="true" className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_82%_30%,rgba(201,151,26,.28),transparent_40%),linear-gradient(135deg,#0B5634,#041C12)]">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(4,28,18,.94)_0%,rgba(4,28,18,.8)_42%,rgba(4,28,18,.25)_80%)] max-lg:bg-[linear-gradient(180deg,rgba(4,28,18,.55)_0%,rgba(4,28,18,.88)_55%,rgba(4,28,18,.95)_100%)]" />
    </div>
  )
}
  ...
  return (
    <section className="relative isolate overflow-hidden bg-brand-900">
      ...
      <div className="mx-auto max-w-site px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
        <p className="text-xs font-bold uppercase tracking-[.08em] text-gold-300">{overline}</p>
...
export function CtaBand({ title = 'Ready to reach new African markets?', text = 'Register your business in minutes, or talk to our team about your trade.' }: { title?: string; text?: string }) {
  return (
    <section className="bg-brand-600">
      <div data-reveal className="mx-auto flex max-w-site flex-col gap-8 px-4 py-16 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div className="max-w-xl">
          <h2 className="font-display text-[clamp(1.75rem,3vw,2.25rem)] font-bold leading-tight text-white">{title}</h2>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Link href="/contact" className={`${button.ghostLight} min-h-12 px-7 text-[15px]`}>Contact us</Link>
        </div>
      </div>
    </section>
  )
...
export function CheckItem({ children, invert = false, className = '', style, reveal = false }: { children: React.ReactNode; invert?: boolean; className?: string; style?: React.CSSProperties; reveal?: boolean }) {
  return (
    <li data-reveal={reveal || undefined} style={style} className={`flex items-start gap-3 ${className}`}>
      <svg viewBox="0 0 20 20" className={`mt-1 h-4 w-4 shrink-0 ${invert ? 'text-gold-300' : 'text-brand-600'}`} fill="currentColor" aria-hidden="true">
