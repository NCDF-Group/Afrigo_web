...
export async function POST(request:Request){if(!features.payments)return featureDisabled();try{
  ...
  if(!payment)return Response.json({ok:false,error:'Paid transaction not found'},{status:409});if(!secret)return Response.json({ok:false,error:'Refunds are temporarily unavailable. Please try again later.'},{status:503})
