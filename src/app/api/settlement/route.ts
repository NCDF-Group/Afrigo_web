...
export async function GET(request:Request){if(!features.payments)return featureDisabled();try{const{user}=await requireUser(request);if(user.user_metadata.role!=='Seller')return Response.json({ok:false,error:'Seller role required'},{status:403});const secret=process.env.PAYSTACK_SECRET_KEY;if(!secret)return Response.json({ok:false,error:'Payout provider is not configured'},{status:503});const response=await fetch('https://api.paystack.co/bank?country=nigeria&currency=NGN',{headers:{Authorization:`Bearer ${secret}`},next:{revalidate:86400}}),result=await response.json();if(!response.ok||!result.status)return Response.json({ok:false,error:'Unable to load banks'},{status:502});return Response.json({ok:true,banks:(result.data||[]).map((item:any)=>({name:item.name,code:item.code})).sort((a:any,b:any)=>a.name.localeCompare(b.name))})}catch(error){return jsonError(error)}}
